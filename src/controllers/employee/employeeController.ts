...
const acceptReviewerRejectedAppraiser  = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (findEmpoloyee.reviewer.reviewer_rejected_value.filter((i: any) => i.value === 'other_recommendation(s)')[0].isChecked === true) {
        const {reviewer} = await Employee.findById(id)
        ...
        const employee = await Employee.updateMany({_id: {$in: id}},
            {
                $set: {
                    ...
                    // "reviewer.reviewer_rating": appraisal.appraiser_rating,
                    // "reviewer.training_recommendation": appraisal.training_recommendation,
                    "appraisal.other_recommendation": reviewer.other_recommendation,
                    // "reviewer.area_of_improvement": appraisal.area_of_improvement,
                    // "reviewer.feedback_questions": appraisal.feedback_questions,
                    // "reviewer.reviewer_acceptance": true,
                    // "reviewerIsChecked": true,
    ...
    if (findEmpoloyee.reviewer.reviewer_rejected_value.filter((i: any) => i.value === 'training_recommendation(s)')[0].isChecked === true) {
        const {reviewer} = await Employee.findById(id)
        ...
        const employee = await Employee.updateMany({_id: {$in: id}},
            {
                $set: {
                    ...
                    // "normalizer.objective_description": reviewer.objective_description,
                    // "reviewer.reviewer_rating": appraisal.appraiser_rating,
                    "appraisal.training_recommendation": reviewer.training_recommendation,
                     // "appraisal.area_of_improvement": reviewer.area_of_improvement,
                     //  "appraisal.feedback_questions": reviewer.feedback_questions,
                    // "reviewer.other_recommendation": appraisal.other_recommendation,
                    // "reviewer.area_of_improvement": appraisal.area_of_improvement,
                    // "reviewer.feedback_questions": appraisal.feedback_questions,
                    // "reviewer.reviewer_acceptance": true,
    ...
    if (findEmpoloyee.reviewer.reviewer_rejected_value.filter((i: any) => i.value === 'Feedback_questionnaire')[0].isChecked === true) {
        const {reviewer} = await Employee.findById(id)
    ...
    if (findEmpoloyee.reviewer.reviewer_rejected_value.filter((i: any) => i.value === 'area_of_improvement')[0].isChecked === true) {
        const {reviewer} = await Employee.findById(id)


        console.log('runnginggggggg')
        const employee = await Employee.updateMany({_id: {$in: id}},
            {
                $set: {
        ...
        const employee = await Employee.updateMany({_id: {$in: id}},
            {
                $set: {
                    ...
                    // "reviewer.reviewer_rating": appraisal.appraiser_rating,
                    // "reviewer.training_recommendation": appraisal.training_recommendation,
                    "appraisal.other_recommendation": normalizer.other_recommendation,
                    // "reviewer.area_of_improvement": appraisal.area_of_improvement,
                    // "reviewer.feedback_questions": appraisal.feedback_questions,
                    // "reviewer.reviewer_acceptance": true,
                    // "reviewerIsChecked": true,
        ...
        const employee = await Employee.updateMany({_id: {$in: id}},
            {
                $set: {
                    ...
                    // "normalizer.objective_description": reviewer.objective_description,
                    // "reviewer.reviewer_rating": appraisal.appraiser_rating,
                    "appraisal.training_recommendation": normalizer.training_recommendation,
                    // "appraisal.area_of_improvement": reviewer.area_of_improvement,
                    //  "appraisal.feedback_questions": reviewer.feedback_questions,
                    // "reviewer.other_recommendation": appraisal.other_recommendation,
                    // "reviewer.area_of_improvement": appraisal.area_of_improvement,
        ...
        const employee = await Employee.updateMany({_id: {$in: id}},
            {
                $set: {
                    ...
                    // "reviewer.other_recommendation": appraisal.other_recommendation,
                    // "reviewer.area_of_improvement": appraisal.area_of_improvement,
                    "appraisal.feedback_questions": normalizer.feedback_questions,
                    // "reviewer.reviewer_acceptance": true,
                    // "reviewerIsChecked": true,
                    // "reviewerIsDisabled": true,
                    // "reviewer.reviewer_rating": appraisal.appraiser_rating,
        ...
        const employee = await Employee.updateMany({_id: {$in: id}},
            {
                $set: {
                    ...
                    // "reviewer.training_recommendation": appraisal.training_recommendation,
                    // "reviewer.other_recommendation": appraisal.other_recommendation,
                    "appraisal.area_of_improvement": normalizer.area_of_improvement,
                    // "normalizer.feedback_questions": reviewer.feedback_questions,
                    // "reviewer.reviewer_acceptance": true,
                    // "reviewerIsChecked": true,
                    // "reviewerIsDisabled": true,
